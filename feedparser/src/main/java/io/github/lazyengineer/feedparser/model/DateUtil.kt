...
object DateUtil {
	...
	private fun parseDate(dateString: String, pattern: String): Date? {
	...
	fun parseTimeString(time: String): Long {
		val timeParts = time.split(":".toRegex()).toTypedArray()
		val millisecondsParts = time.split(".")
		val milliseconds = if (millisecondsParts.size > 1) millisecondsParts.last().toLong() else 0L

		val timeInMilliseconds = timeParts.reversedArray().mapIndexed { index, part ->
			part.toDouble() * 60.toDouble().pow((index.toDouble()))
		}.reduce { sum, element ->
			sum + element
		}.toLong() * 1000
